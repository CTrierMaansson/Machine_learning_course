...
en.model<- train(
                  life_exp ~., 
                  data = train.data, 
                  method = "glmnet", #Bruges til regularized regression
                  trControl = trainControl("cv", number = 10),
                  preProc=c("center", "scale"),
                  tuneLength=10 #Styre at der køres 10 af hver hyperparameter og undersøger dem på kryds og tværs
                   )
#Alpha styrer hvilken regularized regression model som bruges, mens lambda styrer penalty
...
# Model coefficients
coef(en.model$finalModel, en.model$bestTune$lambda) #Gøres på denne måde for at få den bedste version
#Man kan se at nogen af dem er nul. 
#Man kan også se at nogen fetaures er meget givet at de giver lavere levealder. Disse skal 
#fjernes da de egentlig bare korrelerer med outcome og er ikke intervenable
...
postResample(en.pred,test.data$life_exp) #Her kan man se hvor godt testing passer med træning
...
) #Det er ridge model fordi alpha = 0
...
#Note replacing tuneLength with tuneGrid
lasso.model<-train(
  life_exp ~., data=train.data, method="glmnet", trControl=trainControl("cv", number=10), preProc=c("center", "scale"), tuneGrid=expand.grid(alpha=1, lambda=lambda)
) #Det er ridge model fordi alpha = 0
...
# Model prediction performance
postResample(lasso.pred,test.data$life_exp)
